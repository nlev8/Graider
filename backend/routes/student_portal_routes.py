...
from backend.services.student_progress_reports import (
    build_class_progress_rank,
    build_student_report_card,
)
from backend.services.student_gradebook import (
    build_class_gradebook,
...
# Wave 5 Slice 2: remediation constants + helpers extracted to
# backend/services/student_remediation.py. Re-imported here for back-compat
# (existing route bodies, callers, and patch() targets keep working).
from backend.services.student_remediation import (  # noqa: F401  (re-export shim)
    ...
    student_has_standard_evidence,
    resolve_red_tier_students,
)
...
def get_class_progress_rank(class_id):
    ...
    try:
        ...
        # Heavy assembly extracted to the service (Wave 5 Slice 3). The cache
        # asymmetry is preserved: only the full payload is cacheable.
        payload, cacheable = build_class_progress_rank(db, class_id, class_name, attempt_mode)
        if cacheable:
...
def get_student_report_card(class_id, student_id):
    ...
    # Assembly (content + submissions + trajectory + breakdown) extracted to
    # the service (Wave 5 Slice 3); auth/checks/name resolution stay here.
    return jsonify(build_student_report_card(
        db, class_id, class_name, student_id, student_name, attempt_mode,
    ))
...
def get_class_gradebook(class_id):
    ...
    # Assembly (roster + content + submissions + canonical grades) extracted to
    # the service (Wave 5 Slice 4a); auth + name resolution stay here.
    return jsonify(build_class_gradebook(db, class_id, class_name, attempt_mode))
...
def get_student_submission_detail(submission_id):
    ...
    """
    db = _get_teacher_supabase()
    # Assembly + the interleaved not-found/not-authorized checks extracted to
    # the service (Wave 5 Slice 4b); it returns (payload, err) so error_response
    # translation + g.teacher_id stay route-side.
    payload, err = build_submission_detail(db, submission_id, g.teacher_id)
    if err is not None:
...
def get_class_assessment_comparison(class_id):
    ...
    # Distribution stats + standards-matrix assembly extracted to the
    # service (Wave 5 Slice 4c); steps 1-5 (auth + validation + cross-class
    # guard) stay here and resolve `found` + content_ids.
    return jsonify(build_assessment_comparison(
        db, class_id, class_name, content_ids, found, attempt_mode,
    ))
...
def _run_personalized_remediation(db, cls_row, target_mode, target_student_ids,
                                  per_student_segments, standard_code, count,
                                  difficulty, dok, class_id):
    """Generate one personalized remediation variant per targeted student
    (parallel, all-or-nothing) and return the Flask JSON response.

    Extracted verbatim from post_remediate (CQ7 god-function split). The
    `if should_personalize:` branch now delegates here; behavior is pinned
    by tests/test_remediation_perstudent.py.
    """
...
def _run_shared_remediation(cls_row, target_mode, target_student_id,
                            target_student_ids, standard_code, count,
                            difficulty, dok, class_id):
    """Generate a single shared remediation set (one prompt, optional
    single-student accommodation) and return the Flask JSON response.

    Extracted verbatim from post_remediate (CQ7 god-function split); the
    route's fall-through shared path now delegates here. Behavior is pinned
    by tests/test_remediation.py.
    """
...
def post_remediate(class_id):
    ...
    if target_mode == 'single_student':
        # Evidence computation extracted to the service (Wave 5 Slice 5);
        # the warning + 400 stay route-side.
        if not student_has_standard_evidence(db, target_student_id, class_content_ids, standard_code):
            _logger.warning(
    ...
    if target_mode == 'red_tier_in_class':
        # Red-tier resolution extracted to the service (Wave 5 Slice 5); the
        # empty->400 + warning stay route-side.
        red_tier = resolve_red_tier_students(
            db, class_id, class_content_ids, class_content_titles, standard_code,
        )
        if not red_tier:
            _logger.warning(
    ...
    if should_personalize:
        return _run_personalized_remediation(
            db, cls_row, target_mode, target_student_ids,
            per_student_segments, standard_code, count, difficulty, dok, class_id,
        )

    return _run_shared_remediation(
        cls_row, target_mode, target_student_id, target_student_ids,
        standard_code, count, difficulty, dok, class_id,
    )
