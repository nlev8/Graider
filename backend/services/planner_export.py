...
def _build_question_figure_part1(question, show_answer, q_type, plt, np, _math):
    """Build the matplotlib figure for the first half of the visual question types
    (number_line, coordinate_plane, geometry shapes, prisms/cylinder, box_plot, bar_chart,
    function_graph). Returns the fig, or None if q_type is not handled here. Extracted
    verbatim (de-indented) from _create_visual_for_question (Code Quality 6→7 split)."""
    fig = None
...
def _build_question_figure_part2(question, show_answer, q_type, plt, np, _math):
    """Build the matplotlib figure for the second half of the visual question types
    (dot_plot, stem_and_leaf, unit_circle, transformations, fraction_model, probability_tree,
    tape_diagram, venn_diagram, histogram, pie_chart). Returns the fig, or None if q_type is
    not handled here. Extracted verbatim (de-indented) from _create_visual_for_question."""
    fig = None
    if q_type == 'dot_plot':
...
def _create_visual_for_question(question: dict, show_answer: bool = False):
    ...
    try:
        ...
        fig = _build_question_figure_part1(question, show_answer, q_type, plt, np, _math)
        if fig is None:
            fig = _build_question_figure_part2(question, show_answer, q_type, plt, np, _math)
...
# ── Wave 6 Slice 1: study-aid render helpers moved verbatim from planner_routes ──
...
def build_platform_export(assessment, platform, template_structure) -> dict | None:
    ...
    keeps request parse + the template-structure file load + jsonify.
    """
    import csv
    import io
    import base64
