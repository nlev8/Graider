...
${signature}`;
  ...
  var handleNlmPreview = async function(materialType) {
    try {
      var mediaTypes = ["audio_overview", "video_overview", "infographic", "data_table", "slide_deck"];
      if (mediaTypes.indexOf(materialType) !== -1) {
        setNlmPreviewData({ type: materialType, mediaUrl: "/api/notebooklm/download/" + materialType + "?inline=1" });
      } else {
        ...
        >
          ...
          <div style={{ padding: activeTab === "results" ? "20px 15px" : "30px", flex: 1, overflowY: "auto" }}>
            <div style={{ maxWidth: activeTab === "results" ? "none" : "1400px", margin: "0 auto" }}>
              ...
              {activeTab === "results" && (
                <ResultsTab
                  getAuthenticityStatus={getAuthenticityStatus}
                  getAIFlagColor={getAIFlagColor}
                  getPlagFlagColor={getPlagFlagColor}
                  status={status}
                  config={config}
                  rubric={rubric}
                  globalAINotes={globalAINotes}
