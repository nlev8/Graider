...
# `backend/extensions.py` raises at startup if Redis is unreachable
# under FLASK_ENV=production. Provision via Railway Add-on → Redis
# (or any managed Redis 6+; Upstash, Render, etc.).
