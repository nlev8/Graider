...
**STATUS: CLOSED 2026-05-19** -> shipped via PR1 #430 (repository module + char net) and PR2 (rewire); supabase_table string dispatch eliminated from the grading pipeline; zero schema change, zero behavior change.
...
## Implementation refinement (2026-05-19, post PR1 #430 + PR2)

PR1 Steps 7/8 sketched `claim_for_grading -> bool` ("claim only if not already claimed") and Steps 9/10 sketched `mark_failed`. The controller corrected three points during PR1 so the refactor stayed strictly behavior-preserving against the pinned characterization net. These notes amend, not rewrite, the step text above; the shipped API is authoritative.

1. `claim_for_grading` mirrors the legacy `_claim_submission_for_grading` exactly: an unconditional 3-field write (`status='grading_in_progress'`, `grading_task_id`, `grading_started_at`) returning None, not a `-> bool` already-claimed guard. The dedup and stale-claim DECISION stays in the pipeline caller `grade_portal_submission_sync`, unchanged from where it lived before, because that decision is pipeline-scoped and has its own direct test caller.

2. `_is_stale_claim` stays entirely in `backend/services/portal_grading.py`. It is not moved into the repository, not imported by it, and not changed. No cycle is introduced (the repository imports nothing from `portal_grading`).

3. `mark_failed(submission_id, error)` writes exactly `{'status': 'failed', 'error_message': str(error)[:500]}` (status is `failed`, not `grading_failed`), mirroring the `on_failure` write byte-for-byte.

PR2 also kept the Celery `on_failure` hook and the assessment-unavailable abort path on `_safe_update_submission` rather than `repository_for(...).mark_failed(...)`: the characterization net and `test_grading_tasks` pin those by patching the `_safe_update_submission` module symbol, and `repo.mark_failed` -> `repo.update` bypasses that symbol, so swapping it there would break the byte-identical net. The DB effect is identical (same table via the enum value, same fields). The `supabase_table` string dispatch is fully gone from the `portal_grading.py` pipeline body (grep gate green); `grading_tasks.py` is outside the grep gate scope.
