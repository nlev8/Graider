...
**Status:** CLOSED 2026-05-17. Shipped via PR #406 (PR1 standards), #407 (PR2 export), #408 (unit_circle fix + 28-branch net), #409 (PR3 prompts). planner_routes.py 6,050 → 4,598 LOC. Left behind per §3: _get_openai_context (reads Flask g), _save_grading_config_for_export (inner from flask import g).
...
- [x] **Step 1: Find every importer of these names from planner_routes**
...
- [x] **Step 2: List the imports the moved functions use**
...
- [x] **Step 1: Write the failing test (targets the desired end-state import path; no Flask client)**
...
- [x] **Step 1: Create `backend/services/planner_standards.py`**
...
- [x] **Step 2: Replace the defs in `planner_routes.py` with a shim import**
...
- [x] **Step 3: Run the new unit test → GREEN**

Run: `source venv/bin/activate && python -m pytest tests/test_planner_standards.py -q`
Expected: PASS (3 passed).

- [x] **Step 4: Regression — route + planner suites unchanged**
...
- [x] Push branch `feature/planner-extraction`, open PR, all 9 required checks green, squash-merge. PR body notes any function left behind per the coupling rule.
...
- [x] **Step 1: Write characterization tests pinning current output (import from current location)**
...
- [x] **Step 2: Run, confirm GREEN against current code**
...
- [x] **Step 1: Write the failing test (no Flask client)**
...
- [x] **Step 2: Create `backend/services/planner_export.py`**
...
- [x] **Step 3: Shim import in `planner_routes.py`**
...
- [x] **Step 4: Both test files GREEN, characterization unchanged**
...
- [x] **Step 1: Caller audit**

Run: `git grep -nE "_build_assignment_prompt|_build_period_differentiation_block" -- ':!docs'`

- [x] **Step 2: Create `backend/services/planner_prompts.py`** with docstring `"""Prompt construction for the planner. Pure string building (no Flask)."""`, the imports the two functions use, and the two functions moved verbatim (lines 1298, 3692).

- [x] **Step 3: Shim import** near line 40 of `planner_routes.py`:
...
- [x] **Step 1:** `source venv/bin/activate && python -m pytest tests/ -q -k "planner or lesson or assessment or assignment or export or standards or prompts or worksheet" 2>&1 | tail -3` → 0 failed.
- [x] **Step 2:** `ruff check backend/` → All checks passed; `wc -l backend/routes/planner_routes.py` (record the reduced LOC).
- [x] **Step 3:** Append a dated "Tier 2 slice 1 — planner_routes extraction shipped" note to `docs/superpowers/specs/2026-03-20-comprehensive-hardening-assessment.md` (Code Quality / Architecture nudge; no multi-model re-score — mechanically test-guarded like Data Integrity Tier 1) and STATUS-stamp this plan CLOSED. List any functions left behind per the coupling rule and why.
