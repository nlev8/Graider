...
| # | Decision | Rationale |
|---|---|---|
| **F1** | **a1 for the 30 DATA captures** (identically-named keyword-only params, body byte-identical). **HYBRID for the 7 import-bound names** (6 grade fns + `ASSIGNMENT_NAME`): kept as a FUNCTION-LOCAL import inside the lifted gsf (a2 for those 7 only). Module-level hoist BANNED (no-ops golden-net `patch('assignment_grader.*')`). | User ratified a1. **Amended during execution (user-approved):** `backend/grading/*` is mypy-strict — threading the 7 fns as params would force loose `Callable` annotations that make the body's `# type: ignore[arg-type]` comments unused (`warn_unused_ignores` → error), forcing body edits that *break* byte-identity. The function-local import keeps their real types → ignores stay valid → body byte-identical. This is the most byte-identical realization of a1 under mypy-strict. |
