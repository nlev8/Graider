...
- [ ] **Step 2:** Re-confirm the 6 handler bodies' external dependencies are exactly `{getActiveAssignment, setActiveAssignment, addToast, config, unitConfig, globalAINotes, lessonPlan, generatedAssignment, generatedAssessment}` + `api.regenerateQuestions` + the 4 own state setters, and that they call no other PlannerTab closures (they may call each other — fine, both in the hook):
```bash
...
PY
```
Expected: props used ⊆ the 9 inputs (plus possibly others — if any NEW prop appears, add it to the hook inputs); `api.*` = `['regenerateQuestions']`. No commit.
...
```jsx
  const {
    ...
    globalAINotes, lessonPlan, generatedAssignment, generatedAssessment,
  });
```
...
- [ ] **Step 4:** Build + full suite: `cd frontend && npm run build` (clean) + `npx vitest run` (all pass, count = floor + 4). If build/`X is not defined`: a handler referenced a value not in the 9 inputs — add it to the hook inputs + the call (do not guess; trace it).
...
- [ ] **Step 7:** Two-stage review (spec-compliance: verbatim handler move + single call + 9 inputs + blocks untouched; then code-quality). Fix findings.
