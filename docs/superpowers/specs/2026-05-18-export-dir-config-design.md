...
## 4. Migration of the 19 sites and the 3 import-time constants
...
The three module-level constants are the one non-mechanical part. `DOCUMENTS_DIR`, `WORKSHEETS_DIR`, and `EXPORT_DIR` are evaluated at import time, which would capture the path before the session test fixture sets the env var. Each is converted from `X = os.path.expanduser(...)` to a call-time module-local zero-argument accessor (for example `def _documents_dir(): return graider_export_dir("Documents")`), and that module's references are updated to call the accessor. Each converted constant gets a characterization check confirming its resolved value with no env var is byte-identical to the prior constant value.
...
**In:** the `backend/paths.py` resolver; migration of all 19 sites; conversion of the 3 import-time constants to call-time accessors; the conftest convergence (single env-var fixture, brittle monkeypatch removed); `tests/test_paths.py`; documenting `GRAIDER_EXPORT_DIR` in CLAUDE.md's environment section.
...
- **Import-time capture reintroducing pollution.** Section 4 converts all 3 constants to call-time accessors; the grep gate proves no residual import-time `expanduser` of the base.
