...
def generate_image(self, request: ImageRequest) -> ImageResponse:
    ...
    try:
        ...
        # callers (e.g. slide_generator) loop across many requests during
        # the breaker's 60s open window. Skip the error emit + Sentry
        # breadcrumb for breaker-open; let it propagate cleanly.
    ...
    # a graceful empty-image path into a 500. Defend, then fall through to
    # the blocked-event detection below.
    images: list[bytes] = []
...
- `test_gemini_generate_image_safe_on_empty_candidates` — mock response with `raw.candidates = []` AND `raw.prompt_feedback` absent (or `block_reason=None`); assert `ImageResponse(images=[])` returned (no `IndexError`), `llm.image.call.blocked` event with `finish_reason="unknown"`. Isolates the no-candidate / no-feedback fallback path from the `prompt_feedback.block_reason` case exercised by the next test.
...
for idx, (slide_index, image_prompt) in enumerate(image_slides):
    try:
        ...
        # Preserve current code's warning log. Filter CircuitBreakerError out of
