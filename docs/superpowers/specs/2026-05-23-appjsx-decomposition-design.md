...
Small slices, lowest-risk-first to prove each new gate before the high-value/high-risk targets — the same discipline that started the SettingsTab decomposition with the small sections, not the 2,300-line classroom.

- **Slice 1 (this spec's detailed first PR): `HelpTab` push-down.** Smallest, fully self-contained, single-consumer cluster — proves the characterization + push-down gate on the lowest-blast-radius seam. Detailed below.
- **Slice 2: a second push-down** of another single-consumer cluster (candidate: tutorial/onboarding overlay state, or another isolated single-tab cluster identified by a consumer audit) — confirms the push-down pattern repeats cleanly.
- **Slice 3: `useRosterManagement` hook** — the headline lever: the ~28-state roster/SIS cluster (Clever/OneRoster/LTI/periods/students/accommodations) shared across Grade/Planner/Settings/Analytics. Lift-to-hook. Done only after the gate is proven, because it is the most cross-cutting cluster (read in the settings-load, analytics-gate, and sorted-periods effects, and consumed by 5 tabs) — the "false domain boundaries" risk is highest here, so its boundary is fixed by a free-variable/consumer inventory first.
- **Slice 4+: remaining shared-state hooks** (grading/results, calendar, etc.) and **auth-gate screen extraction** (login/loading/not-approved → an `AuthGate`/`AppRouter`, pure-render, JSX-parity-gated) as a parallel track.
- **Context:** considered only after the above; not committed to in this design.

Each slice past Slice 1 re-audits its boundary against freshly-merged main (line numbers and consumer sets drift as prior slices land) and gets its own task in the implementation plan.
...
- **First slice:** Codex argued (with file:line evidence) against starting at the high-value-but-cross-cutting roster cluster, for a small self-contained seam first; reconciled to **HelpTab**, unified with Gemini's push-down principle (single-consumer state → eliminate into its tab). Roster/SIS is retained as the headline lever at Slice 3, once the gate is proven.
