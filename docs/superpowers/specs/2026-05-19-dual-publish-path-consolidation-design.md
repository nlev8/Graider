...
## 11. Implementation refinement (2026-05-19, post PR1 #430 + PR2)

Sections 3, 4, and 6 describe an idealized repository API. During PR1 (the repository module + characterization net) the controller corrected three points to keep the refactor strictly behavior-preserving against the pinned characterization net. These notes amend, but do not rewrite, the idealized text above; the shipped API is authoritative.

1. `claim_for_grading` mirrors the legacy `_claim_submission_for_grading` exactly. It is an unconditional 3-field write (`status='grading_in_progress'`, `grading_task_id`, `grading_started_at`) and returns None, not the `-> bool` "claim only if not already claimed" shape section 3 sketched. There is no already-claimed guard and no TTL check inside the repository. The dedup and stale-claim DECISION stays in the pipeline caller `grade_portal_submission_sync`, exactly where it lived before, because that decision is pipeline-scoped and has its own direct test caller.

2. `_is_stale_claim` stays entirely in `backend/services/portal_grading.py`. It is not moved into the repository, not imported by the repository, and not changed. It is pipeline-scoped, has its own test caller, and the TTL/reclaim semantics are unchanged.

3. `mark_failed(submission_id, error)` writes exactly `{'status': 'failed', 'error_message': str(error)[:500]}` (status is the string `failed`, not `grading_failed`), mirroring the Celery `on_failure` write byte-for-byte.

One further PR2 detail: the Celery `on_failure` hook and the assessment-unavailable abort path in `grading_tasks.py` intentionally keep calling `backend.services.portal_grading._safe_update_submission` rather than `repository_for(...).mark_failed(...)`. The characterization net and `test_grading_tasks` pin those hooks by patching the `_safe_update_submission` module symbol and asserting its call; `repo.mark_failed` -> `repo.update` bypasses that symbol, so swapping it there would break the byte-identical net (a behavior-preserving violation). The database effect is identical either way (same table via the enum value, same fields). The `supabase_table` string dispatch is fully removed from the grading pipeline body in `portal_grading.py` (grep gate green); `grading_tasks.py` is out of the grep gate's scope.
